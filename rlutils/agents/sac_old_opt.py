...
class SacAgent:
    def __init__(self, 
        ...
        self.pi = SequentialNetwork(code=net_code_pi).to(self.device)
        self.optimiser_pi = optim.Adam(self.pi.parameters(), lr=self.P["lr_pi"])
        self.Q, self.Q_target, self.optimiser_Q = self._make_Q(net_code_Q)
        self.Q2, self.Q2_target, self.optimiser_Q2 = self._make_Q(net_code_Q)
    ...
    def update_on_batch(self):
        ...
        next_Q_values[nonterminal_mask] = torch.min(
        ...
        # Compute target = reward + discounted [Q_target(s', a') - entropy term].
    ...
    def _make_Q(self, net_code_Q):
        ...
        Q = SequentialNetwork(code=net_code_Q).to(self.device)
        optimiser_Q = optim.Adam(Q.parameters(), lr=self.P["lr_Q"])
        Q_target = SequentialNetwork(code=net_code_Q).to(self.device)
