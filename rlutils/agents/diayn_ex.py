...
class DIAYN(SAC):
    ...
    def _sample_z(self):
        """Samples z from p(z), using probabilities in self._p_z.""" # Sample z uniform randomly
        return np.random.choice(self._num_skills, p=self._p_z)

    def _split_obs(self):
        return tf.split(self._obs_pl, [self._Do, self._num_skills], 1) # Split observation into state vector and one-hot skill code
    ...
    def _init_critic_update(self):
        ...
        if self._add_p_z:                                                                   # NOTE: Remove for e.g. MountainCar when early termination good.
            reward_pl -= log_p_z
    ...
    def _train(self, env, policy, pool):
        ...
        with self._sess.as_default():
            ...
            for epoch in gt.timed_for(range(self._n_epochs + 1),
                ...
                path_length_list = []
                z = self._sample_z()
                aug_obs = concat_obs_z(observation, z, self._num_skills)
                ...
                for t in range(self._epoch_length):
                    ...
                    aug_next_ob = concat_obs_z(next_ob, z,
                                                     self._num_skills)
                    path_length += 1
                    path_return += reward
